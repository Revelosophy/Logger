    ...
    <div class="card border-dark text-dark bg-secondary" style="margin:20px;">
        <div class="card border-dark text-dark" style="margin:20px;">
            ...
            @if (!is_null($post->image_id))
                <img class="card-img-top" style ="width:50%; margin-left:auto; margin-right:auto; margin-top:20px;  display:block;"src="{{DB::table('images')->where('id',$post->image_id)->value('link')}}" alt="Responsive image">
            @endif
            
        <div class="card-body"><p class="card-text">{{$post->text}}</p></div>
